...
<title>Search - MyReflections - Naresh</title>
...
<body>
	...
	<div id="mainbody" style="height:400px;margin-right:150px;margin-left:150px;background-color:green;border-style:Solid;border-color:white;border-left-width:0px;border-right-width:0px;border-top-width:0px;margin-bottom-width:0px;">
		...
		<div id="content" style="height:370px;width:80%;margin-top:0px;float:left;background-color:white">
		  <% String url = request.getScheme()+"://"+request.getServerName()+":"+request.getServerPort()+request.getContextPath()+"/getArticle.action?articleName="; %>
			<s:set name="isNotEmpty" value="articlePresent"/>
			<s:if test="%{#isNotEmpty !=''}">
		    	<display:table name="articlesList" pagesize="5" requestURI="findArticles.action">
		    		<display:setProperty name="basic.msg.empty_list" value="" />
		    		<display:column property="articleName" title="Article Name" sortProperty="true" href="<%= url %>" paramId="articleName" />
		    		<display:column property="articleContent" title="Content" />
		    		<display:column property="articlePostedBy" title="Posted By" />
